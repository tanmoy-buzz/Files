...
customer_number = session:getVariable("cc_customer")
uniqueid = session:getVariable("uniqueid")
...
function play_audio(ivr_nums)
	...
	if (ivr_nums ~= "") then
			...
			if data["vb_dtmf"] then
				callback_digit = "-"
				for key, value in pairs(data["vb_dtmf"]) do
                    session:consoleLog("info",key)
					if value["is_callback"] == true then
						callback_digit = key
						break
                    end
				end
                session:consoleLog("info",callback_digit.."cb"..digits)
				if callback_digit == digits then	
					session:execute("socket","127.0.0.1:8086 async full")
				else
