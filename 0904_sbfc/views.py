...
from .models import (
	...
	USER_TIMEZONE,DiallerEventLog,SMSGateway,
	...
	InGroupCampaign,CampaignPriority, EmailTemplate, Appointments, Daemons,PasswordChangeLogs,PasswordManagement
)
from flexydial.constants import (Status, REPORTS_LIST, TRUNK_TYPE, UserStatus, DIAL_KEY_OPTIONS, PROTOCOL_CHOICES, CALL_MODE,
	...
	DISPO_FIELD_TYPE, TIMER_SEC, PAGINATE_BY_LIST, CONTACT_STATUS, MONTHS, Gateway_Mode, MODULES as flexydial_modules,
	STRATEGY_CHOICES,COUNTRY_CODES, SBFC_STATIC_CRM_DATA,PasswordChangeType,SHOW_DISPOS_TYPE,TTS_LANGS)

from crm.models import (CrmField, TempContactInfo, Phonebook,KycDocuments,
			Contact,CrmField,ContactInfo, DownloadReports, LeadBucket, AlternateContact,Pincode)
from crm.serializers import (AgentCrmFieldSerializer,TempContactInfoSerializer, ContactSerializer,CssContactSerializer, AssignedContactInfoSerializer, ContactListSerializer, LeadBucketSerializer, AlternateContactSerializer,PincodeSerializer)
...
from flexydial.views import (check_permission, get_paginated_object, data_for_pagination, get_active_campaign, 
	data_for_vue_pagination, sendSMS, csvDownloadTemplate, create_admin_log_entry, sendsmsparam,mapmyindia_token_generator)
...
from .utility import (redirect_user, get_object, get_pre_campaign_edit_info,
	...
	save_report_column_visibility, get_report_visible_column,save_email_log, get_used_did, get_used_did_by_pk,convert_into_timeformat, email_connection, check_non_admin_user, SqlServerConnection, getDaemonsStatus, upload_holiday_dates,ApiSchemeAndPincodeConnectionCall)
...
from dialer.dialersession import (dial,twinkle_session,hangup,autodial_session,autodial_session_hangup,
	...
	fs_administration_hangup,fs_eavesdrop_activity,fs_set_ibc_contact_id,inbound_stiky_agent_bridge,click_to_call_initiate,on_break,check_ibc_cust_status)
import glob
import pyodbc
		...
		if serializer.is_valid():
			...
			if user is not None:
				if user.is_active:
					...
					# CallDetail.objects.filter(user__username=user.username,created__gte=date.today()).count()

					AGENTS[user.extension]['call_count'] = CallDetail.objects.filter(user__username=user.username,created__gte=date.today()).count()
					updated_agent_dict = pickle.loads(settings.R_SERVER.get("agent_status") or pickle.dumps(AGENTS))
					updated_agent_dict[user.extension] = AGENTS[user.extension]
					settings.R_SERVER.set("agent_status", pickle.dumps(updated_agent_dict))
					...
					if username in PASSWORD_ATTEMPTS and PASSWORD_ATTEMPTS[username] >= password_obj.max_password_attempt:
				...
				if user_obj.exists():
					if password_obj:
						pswdatmpts=password_obj.max_password_attempt
						if username in PASSWORD_ATTEMPTS:
							if PASSWORD_ATTEMPTS[username] >= password_obj.max_password_attempt:
								...
								pswdatmpts=password_obj.max_password_attempt - PASSWORD_ATTEMPTS[username]
								PASSWORD_ATTEMPTS[username] += 1
						else: 
							PASSWORD_ATTEMPTS[username] = 1 
						settings.R_SERVER.set('password_attempt_status',pickle.dumps(PASSWORD_ATTEMPTS))
		error_dict = {"error":"Username or Password is incorrect. You have "+str(pswdatmpts)+" attempts left",'forgot_password':forgot_password}
		return Response(error_dict)
...
class CampaignLiveDataView(LoginRequiredMixin, APIView):
	...
	def get(self, request):
		...
		for campaign in queryset:
			...
			start_date_qr = datetime.now().date()
			end_date_qr = start_date_qr + timedelta(days=1)
			campaign_dict['totalcalls_today'] = CallDetail.objects.filter(campaign_name=campaign['name'] ,updated__gte=start_date_qr,updated__lte=end_date_qr,dialed_status='Connected').count()
			
			# vc_blaster_calls = CallDetail.objects.filter(campaign_name=campaign['name'],callmode="voice-blaster",updated__date=datetime.now().date(),dialed_status__in = ['Full Audio Played']).count()
			...
			campaign_dict['totalcalls_today'] =+ vc_blaster_calls
...
class TrunkGroupCreateEditApiView(LoginRequiredMixin,APIView):
	...
	def get(self, request, pk=None, format=None):
		...
		trunk_list = list(all_trunk.annotate(text=F('name')).values("text","id","did_range","dial_digits"))
...
class CamapignScheduleCreateApiView(LoginRequiredMixin, generics.CreateAPIView):
	...
	def get(self, request, **kwargs):
		...
		context = {'request': request, "audio_files": audio_files, 'campaign_schedule_list': campaign_schedule,"Except_mode": Except}
		context = {**context, **kwargs['permissions']}
		return Response(context)
...
class CampaignScheduleEditApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request, pk, format=None, **kwargs):
		...
		audio_files = AudioFile.objects.all()
		context = {'request': request, "campaign_schedule": campaign_schedule, "audio_files": audio_files,"Except_mode": Except}
		context = {**context, **kwargs['permissions']}
		return Response(context)
		...
		if campaign_serializer.is_valid():
			...
			if trunk_detail:
				...
				for trunk in trunk_detail:
					...
					if "range_mismatch_error" in trunk:
						del trunk["range_mismatch_error"]
...
class CallDetailReportView(LoginRequiredMixin,APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		if request.user.is_superuser or admin:
			...
			user_list = User.objects.all().prefetch_related(
		...
		"calldetail":['campaign_name','user','full_name','supervisor_name','phonebook','customer_cid','customer_full_name','location','contact_id','uniqueid',
		...
		'customer_info':['pincode','lead_source','lead_created_date','flexydial_insert_date']}
...
class PendingCallbackCallView(LoginRequiredMixin, generics.ListAPIView):
	...
	def post(self,request):
		...
		if selected_campaign:
			...
			queryset = queryset.filter(schedule_time__lte=date.today()-timedelta(days=threshold), campaign=camp.name)
		if selected_user:
...
class PendingAbandonedCallView(LoginRequiredMixin, generics.ListAPIView):
	...
	def post(self,request):
		...
		if selected_campaign:
			...
			queryset = queryset.filter(created_date__lte=date.today()-timedelta(days=threshold), campaign=camp.name)
		if selected_user:
...
class AgentHomeApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request, **kwargs):
		...
		total_agentcalls = CallDetail.objects.filter(user=request.user).exclude(cdrfeedback=None).aggregate(total_agentcalls_today=Count('id', filter=Q(created__gte=date.today())),total_agentcalls_month=Count('id', filter=Q(created__gte=first_day_of_month)))
		...
		access_token = ''
		mapmyindia_token_generator()
		MAPMYINDIA = pickle.loads(settings.R_SERVER.get("mapmyindia") or pickle.dumps({}))
		if 'access_token' in MAPMYINDIA:
			access_token = MAPMYINDIA['access_token']
		else:
			print("Map MY India aceess token Generation Failed")
		...
		'user':login_agent_data,'mapmyindia_access_token':access_token,'non_agent_user':non_agent_user}
...
class DiallerLogin(LoginRequiredMixin, APIView):
	...
	def post(self, request):
		...
		holidays = Holidays.objects.filter(status="Active",holiday_date=datetime.today()).first()
		holiday_obj = Holidays.objects.filter(status='Active',campaigns__name=campaign.name,except_type="1",holiday_date=datetime.today())
		...
		if campaign.portifolio:
			...
			total_camp_assigned_calls = Contact.objects.filter(user=request.user.username, campaign="GoldGenie").count()
		user_role_list = []
		...
		camp_data_1 = campaign_data
		if holidays and holidays.status =='Active':
			holid_type = holidays.except_type	
			if holid_type=="2":
				campaign_data.update({"dial_method":{'manual': False, 'inbound': True, 'outbound': False, 'ibc_popup': False, 'no_agent_audio': False, 'sticky_agent_map': False}})
			if holid_type=="3":
				campaign_data.update({"dial_method":{'manual': camp_data_1['dial_method']['manual'], 'inbound': False, 'outbound':camp_data_1['dial_method']['outbound'], 'ibc_popup': False, 'no_agent_audio': False, 'sticky_agent_map': False}})
		camp_obj_dial_methods=campaign.schedule.except_type if campaign.schedule else 'noexcept'
		# camp_data_1=campaign_data
		today_day=datetime.today().strftime('%A')
		today_time=datetime.today().strftime("%I:%M %p")#to obj covert
		if campaign.schedule and campaign.schedule.status == 'Active':
			check_cond=campaign.schedule.schedule[today_day]['stop_time']#to obj covert
			if check_cond!='':
				check_co=datetime.strptime(check_cond, '%I:%M %p')
				today1=datetime.strptime(today_time, '%I:%M %p')
				if check_co<today1:
					if camp_obj_dial_methods=="2":	
						campaign_data.update({"dial_method":{'manual': False, 'inbound': camp_data_1['dial_method']['inbound'], 'outbound': False, 'ibc_popup': False, 'no_agent_audio': False, 'sticky_agent_map': False}})
					elif camp_obj_dial_methods=="3":
						campaign_data.update({"dial_method":{'manual': camp_data_1['dial_method']['manual'], 'inbound': False, 'outbound':camp_data_1['dial_method']['outbound'], 'ibc_popup': False, 'no_agent_audio': False, 'sticky_agent_map': False}})
					else:
						return Response({"error":"This campaign is not available in this time period,please coordinate \ with your administrator or login to the another campaign."},status=403)
		# if campaign.dial_method['thirdparty_data_push']:#checking gold genie
		if ('thirdparty_data_push' in campaign.dial_method) and (campaign.dial_method['thirdparty_data_push']):
			try:
				if 'employee_code' not in crm_fieds_data['other_details']:
					#this will execute in client push
					oth=crm_fields[-1]
					y = oth['section_fields']
					cpy1={'size': '100', 'field': 'employee_code', 'unique': False,
						 'options': '', 'db_field': 'employee_code','editable': False, 'priority': 100, 'required': False, 'crm_color': '#392C70', 'field_type': 'text','field_status': 'Active'}
					cpy2={'size': '100', 'field': 'employee_name', 'unique': False, 'options': '', 'db_field': 'employee_name',
						 'editable': False, 'priority': 100, 'required': False, 'crm_color': '#392C70', 'field_type': 'text',
						 'field_status': 'Active'}
					cpy3={'size': '100', 'field': 'employee_role', 'unique': False, 'options': '', 'db_field': 'employee_role',
						 'editable': False, 'priority': 100, 'required': False, 'crm_color': '#392C70', 'field_type': 'text',
						 'field_status': 'Active'}
					y.append(cpy1)
					y.append(cpy2)
					y.append(cpy3)
					print("adding crm fields", {'crm_fields': crm_fields})
					print("sending crm data fields", {'crm_fieds_data': crm_fieds_data})
					crm_fieds_data.update(
						{'other_details': {'employee_code': None, 'employee_name': None, 'employee_role': None, }})
					print("adding crm data fields", {'crm_fieds_data': crm_fieds_data})
			except Exception as e:
				print("the campaign is gold genie but no fields of other details",str(e))
...
class MaunalDialListAPIView(LoginRequiredMixin, APIView):
	...
	login_url = '/'
	def post(self, request):
		print("inside MaunalDialListAPIView")
...
class ManualDial(LoginRequiredMixin, APIView):
	...
	login_url = '/'
	def post(self, request):
		print('coming in manual dial class')
		AGENTS = {}
		AGENTS = pickle.loads(settings.R_SERVER.get("agent_status") or
				pickle.dumps(AGENTS))
		status = {}
		...
		if dial_number:
			...
			status = dial(session_details['variable_sip_from_host'],
				...
				contact_id=contact_id, callmode=callmode,lead_preview=lead_preview, man_unique_id=man_unique_id,user_id=request.user.id,req=request)
	...
	def post(self, request):
		...
		if temp_contact:
			...
			if temp_contact:
				...
				contact_info = ContactSerializer(temp_contact,context={'request': request,'campaign':campaign}).data
				if call_type:
...
class DispoSubmit(LoginRequiredMixin, APIView):
	...
	def post(self, request):
		...
		total_agentcalls_today = CallDetail.objects.filter(user=request.user, created__gte=start_date,created__lt =end_date).annotate(feedback_count=Count('cdrfeedback')).filter(feedback_count__gt=0).count()
		...
		today = datetime.today()
		first_day_of_month = today.replace(day=1)
		total_agentcalls_month = CallDetail.objects.filter(user=request.user, created__gte = first_day_of_month).exclude(cdrfeedback=None).count()
		...
		AGENTS[request.user.extension]['call_count'] = CallDetail.objects.filter(user__username=request.user.username,created__gte=start_date,created__lt =end_date).count()
...
class AutodialCustomerDetail(APIView):
	...
	def post(self, request):
		...
		if dial_number and dial_number !="null":
			...
			#data = get_data_from_clinet({"dial_number":dial_number})
...
def get_data_from_clinet(data) :
	...
	try:
		if conn :
			if "dial_number" in data:
				query = ("select Customer_Name, Mobile_No, Permanent_Address,Loan_No,Loan_Amt,Total_Outstanding,EMI_Amt,Last_EMI_Paid_Date,Loan_Status,Charges from FLEXYDIAL_DETAILS where Mobile_no='{}'").format(data["dial_number"][-10:])
			if "loan_no" in data:
				query = ("select Customer_Name, Mobile_No, Permanent_Address,Loan_No,Loan_Amt,Total_Outstanding,EMI_Amt,Last_EMI_Paid_Date,Loan_Status,Charges from FLEXYDIAL_DETAILS where Loan_No='{}'").format(data["loan_no"][-10:])
			result = pd.read_sql(query,conn).to_dict('records')
			if result != []:
				customer_dict['customer_information'] = {}
				for res in result:
					temp_section = {}
					temp_field = {}
					for k,v in res.items():
						temp_field[k.lower()] = v
					temp_section['customer_information'] = temp_field
					customer_list.append(temp_section)
					customer_info_list.append(temp_field)
			conn.close()
			return {'contact_info':customer_list, 'customer_detail':customer_info_list}
		else:
			return {'contact_info':[], 'customer_detail':[]}
	except Exception as e:
		return {'error':str(e)}
...
class InboundCustomerDetail(APIView):
	...
	def post(self, request):
		try:
			...
			count = Campaign.objects.filter(name=campaign, dial_method__third_party_data_fetch=True).count()
			if count:
...
class SearchIboundCustomerDetailByMobile(APIView):

	def post(self, request):
		try:
			search_by_field = request.POST.get("search_field","")
			search_value = request.POST.get("search_value", "")
			if search_by_field == "mobile_no":
				data = get_data_from_clinet({"dial_number":search_value})
			else:
				data = get_data_from_clinet({"loan_no":search_value})
			# data=customer_detials(campaign,dial_number,campaign_object)
			return JsonResponse(data)
		except Exception as e:
			import sys
...
class SetIbcContactId(APIView):
	...
	def post(self,request):
		...
		callmode = request.POST.get('callmode', '')
		third_party_data_fetch = request.POST.get('third_party_data_fetch','')
		...
		status = fs_set_ibc_contact_id(switch_ip,ic_number=ic_number,contact_id=contact_id,extension=request.user.extension,
			dialed_uuid=dialed_uuid, callmode=callmode,campaign_obj=campaign_obj, third_party_data_fetch=third_party_data_fetch)
		if contact_id:
...
def rec_check_agent_availabilty(request):
	...
	try:
		...
		if diff_in_seconds < timedelta(seconds=int(status.get('c_max_wait_time',25))):  
			inbound_uuids = pickle.loads(settings.R_SERVER.get("inbound_status") or pickle.dumps({}))
...
class CustomerInfoAPIView(APIView):
	...
	def get(self, request):
		...
		call_timestamp = request.GET.get('call_timestamp',None)
		if sip_extension =='':
			sip_extension = request.user.extension
		if call_timestamp==None or call_timestamp=='':
			call_timestamp = int(str(round(time.time() * 1000))[:13])
		else:
			call_timestamp = call_timestamp[:13]
		AGENTS = pickle.loads(settings.R_SERVER.get("agent_status") or pickle.dumps(AGENTS))
...
class AgentLiveDataAPIView(APIView):
	...
	def get(self,request,**kwargs):
		...
		if campaign != "":
			...
			active_appointments = Appointments.objects.filter(campaign=campaign, user=user).count()
			...
			if campaign_obj.portifolio:
				...
				contact_count = Contact.objects.filter(user=user.username, campaign='GoldGenie', phonebook__status='Active').aggregate(notdialed_assigned_calls=Count('status', filter=Q(status__in=['Queued','NotDialed','Locked'])))
		...
		context = {"total_callbacks":total_callbacks,'notifications':notifications,
						...
						'total_abandonedcalls_cc':total_abandonedcalls_cc,'reconfirm_callbacks_cc':active_appointments}
...
class GetTotalCallbacks(LoginRequiredMixin, APIView):
	...
	def get(self, request):
		...
		page=int(request.GET.get('page',1))
		paginate_by=int(request.GET.get('paginate_by',10))
		callback_filter_date = request.GET.get('callback_filter_date','')
		user_obj = request.user
		campaigns = [campaign_name['name'] for campaign_name in user_obj.active_campaign]
		...
		if callback_filter_date:
			callback_date = datetime.strptime(callback_filter_date,"%Y-%m-%d").date()
			filter_dict['schedule_time__date'] = callback_date
		cb_obj = CallBackContact.objects.filter(Q(user=user_obj.extension,campaign__in=campaigns)
			|Q(campaign__in =campaigns, callback_type='queue'),**filter_dict)
...
class GetActiveCallbacks(LoginRequiredMixin, APIView):
	...
	def post(self,request):
		...
		page=int(request.POST.get('page',1))
		paginate_by=int(request.POST.get('paginate_by',10))
		callback_filter_date = request.POST.get('callback_filter_date','')
		user_obj = request.user
		campaign = request.POST.get("campaign_name","")
		if callback_filter_date:
			callback_date = datetime.strptime(callback_filter_date,"%Y-%m-%d").date()
			campaign_ac_obj = CurrentCallBack.objects.filter(Q(user=user_obj.extension,campaign = campaign)
						|Q(callback_type='queue',campaign=campaign, user=None),Q(schedule_time__gte=callback_date)).exclude(callmode="inbound", user=None).exclude(status='Locked').order_by('-schedule_time')
		else:
			campaign_ac_obj = CurrentCallBack.objects.filter(Q(user=user_obj.extension,campaign = campaign)
		...
		paginate_obj=get_paginated_object(campaign_ac_obj,page,paginate_by)	
		if campaign_ac_obj:
			campaign_active_callback = CurrentCallBackSerializer(paginate_obj, many=True).data
		return JsonResponse({'total_records': paginate_obj.paginator.count,'total_pages': paginate_obj.paginator.num_pages,
			'page': paginate_obj.number,'has_next': paginate_obj.has_next(),
			'has_prev': paginate_obj.has_previous(),'start_index':paginate_obj.start_index(), 
			'end_index':paginate_obj.end_index(),"active_callback":campaign_active_callback},status=200)

class GetReconfirmAppointments(LoginRequiredMixin, APIView):
	"""
		This view is used to give the appointments calls 
	"""
	login_url = '/'
	def post(self,request):
		campaign_reconfirm_calls = []
		campaign = request.POST.get('campaign_name',"")
		reconfirm_obj = Appointments.objects.filter(campaign=campaign, user=request.user).order_by('appointment_date')
		campaign_reconfirm_calls = AppointmentsCallsSerializer(reconfirm_obj,many=True).data
		return JsonResponse({"active_appointments":campaign_reconfirm_calls},status=200)
...
class GetCampaignTotalCallbacks(LoginRequiredMixin, APIView):
	...
	def post(self, request):
		...
		page = int(request.POST.get('page' ,1))
		paginate_by = int(request.POST.get('paginate_by', 10))
		callback_filter_date = request.POST.get('callback_filter_date',None)
		...
		campaign_tc_obj = CallBackContact.objects.filter(Q(user=user_obj.extension,campaign=campaign)
				|Q(campaign = campaign, callback_type='queue',user=None),**filter_dict).exclude(callmode="inbound", user=None)
		paginate_obj=get_paginated_object(campaign_tc_obj,page,paginate_by)
		if paginate_obj:
			campaign_total_callback = CallBackContactSerializer(campaign_tc_obj, many=True).data
		return JsonResponse({'total_records': paginate_obj.paginator.count,'total_pages': paginate_obj.paginator.num_pages,
			'page': paginate_obj.number,'has_next': paginate_obj.has_next(),
			'has_prev': paginate_obj.has_previous(),'start_index':paginate_obj.start_index(), 
			'end_index':paginate_obj.end_index(),"campaign_total_callback":campaign_total_callback},status=200)
...
class GetTotalCallsPerDay(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		...
		contact_filter_dict ={}
		if column_name and search_by and column_name != 'first_name':	
			filter_dict[column_name] = search_by
		if disposition:
		...
		if column_name == 'first_name' and 'cdrfeedback__primary_dispo' not in filter_dict:
			contact_filter_dict['customer_raw_data__customer_details__first_name'] = search_by
			contact_ids = list(Contact.objects.filter(**contact_filter_dict).values_list('id',flat=True))
			total_callsperday_obj = total_callsperday_obj.filter(contact_id__in=contact_ids)
		paginate_obj = get_paginated_object(total_callsperday_obj, page, paginate_by)
		agent_callperday = CallDetailReportFieldSerializer(paginate_obj,many=True).data
		return Response({'total_records': paginate_obj.paginator.count,'total_pages': paginate_obj.paginator.num_pages,
...
class GetTotalCallsPerMonth(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		...
		if column_name == 'first_name' and 'cdrfeedback__primary_dispo' not in filter_dict:
			contact_filter_dict['customer_raw_data__customer_details__first_name'] = search_by
			contact_ids = list(Contact.objects.filter(**contact_filter_dict).values_list('id',flat=True))
			queryset = queryset.filter(contact_id__in=contact_ids)
...
class GetKYCDocuments(LoginRequiredMixin,APIView):
	login_url= '/'
	def post(self,request):
		column_name = request.POST.get('column_name',None)
		search_by = request.POST.get('search_by',None)
		campaign_name = request.POST.get('campaign_name','')
		campaing_crm_ref_no = bool(CrmField.objects.filter(campaign__name=campaign_name).values('crm_fields__appointment_details__crm_ref_no'))
		kyc_data = []
		filter_dict = {}
		filter_dict['campaign']=campaign_name

		if campaing_crm_ref_no:
			kyc_crm_ref_nos = list(KycDocuments.objects.values_list("crm_ref_no",flat=True))
			kyc_crm_ref_nos = list(map(str,kyc_crm_ref_nos))
			contact_crm_ref_nos = list(Contact.objects.values_list("customer_raw_data__appointment_details__crm_ref_no",flat=True))
			required_crm_ref_nos = list(set([value for value in contact_crm_ref_nos if value in kyc_crm_ref_nos]))
			filter_dict['customer_raw_data__appointment_details__crm_ref_no__in']=required_crm_ref_nos
			queryset = Contact.objects.filter(**filter_dict)
			kyc_data = list(queryset.values("numeric","customer_raw_data__customer_details__first_name","customer_raw_data__customer_details__last_name","customer_raw_data__customer_details__middle_name","customer_raw_data__appointment_details__crm_ref_no"))
			kyc_data=sorted(kyc_data, key=lambda x: x['customer_raw_data__appointment_details__crm_ref_no'], reverse=True)
			if column_name =="phone_number" and search_by:
				kyc_data = list(queryset.filter(numeric=search_by).values("numeric","customer_raw_data__customer_details__first_name","customer_raw_data__customer_details__last_name","customer_raw_data__customer_details__middle_name","customer_raw_data__appointment_details__crm_ref_no"))

			elif column_name =="first_name" and search_by:
				kyc_data = list(queryset.filter(customer_raw_data__customer_details__first_name__icontains=search_by).values("numeric","customer_raw_data__customer_details__first_name","customer_raw_data__customer_details__last_name","customer_raw_data__appointment_details__crm_ref_no","customer_raw_data__customer_details__middle_name"))

			elif column_name =="middle_name" and search_by:
				kyc_data = list(queryset.filter(customer_raw_data__customer_details__middle_name__icontains=search_by).values("numeric","customer_raw_data__customer_details__first_name","customer_raw_data__customer_details__last_name","customer_raw_data__appointment_details__crm_ref_no","customer_raw_data__customer_details__middle_name"))
	 
			elif column_name =="last_name" and search_by:
				kyc_data = list(queryset.filter(customer_raw_data__customer_details__last_name__icontains=search_by).values("numeric","customer_raw_data__customer_details__first_name","customer_raw_data__customer_details__last_name","customer_raw_data__appointment_details__crm_ref_no","customer_raw_data__customer_details__middle_name"))

		return Response({'context':kyc_data})
...
class GetAgentDispoCount(LoginRequiredMixin, APIView):
	...
	def get(self,request):
		...
		if fetch_type == 'CallsPerMonth':
			...
			dispo_data = CdrFeedbck.objects.filter(calldetail__user=request.user, calldetail__created__gte=start_date,calldetail__created__lt=end_date).values('primary_dispo').order_by('primary_dispo').annotate(count = Count('primary_dispo'))
...
class UpdateSqlServerContactInfo(APIView):
	...
	def post(self,request):
		...
		if mobile_no:
			...
			primary_dispo = request.POST.get('primary_dispo','')
			print(segment,department,query_type,query_category,primary_dispo)
			conn = SqlServerConnection()
			try:
				if conn:
					conn.cursor().execute("update FLEXYDIAL_DETAILS set Segment_segregation=?, Department=?, Query_type=?, Query_Category=?, Status_Of_Call=? where Mobile_no=?",
						segment, department, query_type, query_category, primary_dispo, mobile_no[-10:]) 
					conn.commit()
					conn.close()
					print("successfully updated to there database")
					return JsonResponse({'success':'Data Updated Successfully'})
				print("connection refused")
				return JsonResponse({'error':'Connection Refused To Update Data'})
			except Exception as e:
				print(e)
				return JsonResponse({'error':str(e)})
		else:
			return JsonResponse({'error':'As number is not present in database. We are unable to update the information'})            
		...
		if users.exists():
			for user in users:
				...
				if user.user_role and user.user_role.access_level == 'Agent':
					...
					ac_obj = AgentActivity.objects.filter(user=user, created__gte=start_date, created__lte=end_date, event='LOGIN').distinct('created__date').order_by('created__date')
					...
					admin_count = AdminLogEntry.objects.filter(created_by=user, created__gte=start_date, created__lte=end_date,action_name='4').distinct('created__date').order_by('created__date').exclude(created__date__in=unique_dates).count()
					...
					ad_obj = AdminLogEntry.objects.filter(created_by=user, created__gte=start_date, created__lte=end_date,action_name='4').distinct('created__date').order_by('created__date')

					unique_dates = ad_obj.values_list('created')
					admin_count = ad_obj.count()
					agent_count = AgentActivity.objects.filter(user=user, created__gte=start_date, created__lte=end_date, event='LOGIN').exclude(created__date__in=unique_dates).distinct('created__date').order_by('created__date').count()
...
class VoiceBlasterCreateAPIView(LoginRequiredMixin, APIView):
	...
	def get(self,request,pk=None, **kwargs):
		...
		if check_non_admin_user(request.user):
			campaigns = Campaign.objects.filter(Q(users=request.user)|Q(group__in=request.user.group.all())|Q(created_by=request.user)).distinct()
		# context['gateways'] = Gateway.objects.all().values('name','id')
		context['gateways'] = SMSGateway.objects.all().values('name','id')
...
class VoiceBlasterEditAPIView(LoginRequiredMixin, APIView):
	...
	def get(self,request,pk=None, **kwargs):
		...
		context['vb_cust_text_data'] = vb_obj.vb_data['vb_custom_audio_tts']
		context['tts_langs'] = TTS_LANGS
		...
		if vb_obj.campaign.exists():
			if crm_obj:
				...
				camp_crm_fields = get_crm_fields_dict(vb_obj.campaign.first().name)
				context['crm_field'] = camp_crm_fields
				crm_fields_list = get_customizable_crm_fields(vb_obj.campaign.first().name)
				default_cols = ['numeric','alt_numeric','first_name','last_name','email','priority']
				# context['all_crm_field'] = camp_crm_fields + default_cols 
				context['all_crm_field'] = crm_fields_list + default_cols 
...
class GetPhonebookDetailsApi(LoginRequiredMixin, APIView):
	...
	def get(self, request):
		...
		for phonebook in phonebooks_list:
			...
			campaign_dict['totalcalls_today'] = Contact.objects.filter(phonebook_id=phonebook.id ,modified_date__gte=datetime.now().date(),status='Dialed').count()
...
class PincodeSearch(LoginRequiredMixin, APIView):
	def get(self, request):
		search_by = request.GET.get('search_by','')
		if search_by:
			response_data =[]
			status = 'UNSERVICEABLE_FLAG'
			status_code =404
			# status_code, fieldmap, response_data  = ApiSchemeAndPincodeConnectionCall('pincodemaster',search_by)
			pin_obj = Pincode.objects.filter(pincode=search_by)
			if pin_obj:
				response_data = PincodeSerializer(pin_obj,many=True).data
				status = 'SERVICEABLE_FLAG'
				status_code = 200
			print('pincodemaster', status_code)
			if status_code == 200:
				return JsonResponse({'data':response_data, 'status':status,"status_code":status_code})
			return JsonResponse({'data':response_data, 'status':status,"status_code":status_code})
		return JsonResponse({'data':"Pincode is Missing", "status":500})


class SchemeSearchApi(LoginRequiredMixin, APIView):
	def get(self, request):
		search_by = request.GET.get('search_by','')
		if search_by:
			status_code, fieldmap, response_data = ApiSchemeAndPincodeConnectionCall('product',search_by)
			print('product', status_code, fieldmap)
			if status_code == 'S':
				return JsonResponse({'data':response_data, 'status':status_code,"field_map":fieldmap})
			return JsonResponse({'data':response_data, "status":status_code,"field_map":fieldmap})
		return JsonResponse({'data':"Scheme is Missing", "status":500,"field_map":''})


class MakeReConfirmCall(LoginRequiredMixin, APIView):
	"""
	This View is to make reconfrimcall 
	"""
	login_url = '/'

	def post(self,request):
		numeric = request.POST.get("re_numeric","")
		campaign = request.POST.get("re_campaign","")
		user = request.POST.get("re_user",None)
		contact_id = request.POST.get("re_contact_id", None)
		ap_obj = Appointments.objects.filter(campaign=campaign,numeric=numeric,contact_id=contact_id)
		if ap_obj:
			if ap_obj[0].status != 'Locked':
				ap_obj.update(status='Locked')
				return JsonResponse({"success":"Ready to make call to "+numeric})
			return JsonResponse({"error":"Its alredy being called by other agent"},status=404)
		return JsonResponse({"error":"Reconfrimcall entry for "+numeric+" not found"},status=404)
...
class GetAgentBreakwisetotal(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		if request.user:
			...
			user_activity = AgentActivity.objects.filter(user=request.user,created__gte=date.today())
...
class AgentAppointmentAvailiblity(LoginRequiredMixin,APIView):

	def get(self,request):
		try:
			URL = settings.SBFC_AGENT_AVAILABILITY_FOR_APPOINTMENT
			PARAMS = json.loads(request.query_params.get('data', '{}'))
			...
			print('AGENT_AVAILABILITY_FOR_APPOINTMENT timeout',timerr)
			return JsonResponse({"error":"Looks like the server is taking to long to respond, please try again in sometime"}, status=408)
		except requests.exceptions.RequestException as e:
			print('Error in get method of Agent appointment availability',e)
			return JsonResponse({"error":"Can't fetch agents at the moment, coordinate with administration"}, status=500)

	def post(self, request):
		try:
			URL = settings.SBFC_AGENT_SCHEDULE_APPOINTMENT
			agent_id = json.loads(request.data.get('agent_id',''))
			appointment_date = request.data.get('appointment_date','')
			if appointment_date and agent_id:
				PARAMS = {'Agent_Id':str(agent_id), 'DateTime':appointment_date}
				r = requests.get(url = URL, headers = PARAMS)
				status = r.json()
				if type(status) == str:
					status = json.loads(status)
				print(agent_id, 'agents book successfully for schedule time', appointment_date)
				return JsonResponse(status)
			return JsonResponse({'error':'Appointment data or agent id is missing'}, status=400)
		except requests.exceptions.Timeout as timerr:
			print('AGENT_SCHEDULE_APPOINTMENT api timeout',timerr)
			return JsonResponse({'error':'Looks like the server is taking to long to respond, please try again in sometime'}, status=408)
		except requests.exceptions.RequestException as e:
			print('Error in post method of Agent appointment availability',e)
			return JsonResponse({'error':'Can fetch agents at the moment, coordinate with administration'}, status=500)

	def put(self, request):
		try:
			URL = settings.SBFC_AGENT_SCHEDULE_UNBLOCK
			agent_ids = request.data.getlist('agent_ids[]',[])
			appointment_date = request.data.get('schedule_time','')
			if appointment_date and agent_ids:
				for agent in agent_ids:
					PARAMS = {'Agent_Id':str(agent), 'DateTime':appointment_date}
					r = requests.get(url = URL, headers = PARAMS)
					status = r.json()
					if type(status) == str:
						status = json.loads(status)
				print(agent_ids, 'agents unblock successfully for schedule time', appointment_date)
				return JsonResponse(status)
			return JsonResponse({'error':'Appointment data or agent id is missing'}, status=400)
		except requests.exceptions.Timeout as timerr:
			print('AGENT_SCHEDULE_UNBLOCK api timeout',timerr)
			return JsonResponse({'error':'Looks like the server is taking to long to respond, please try again in sometime'}, status=408)
		except requests.exceptions.RequestException as e:
			print('Error in put method of Agent appointment availability',e)
			return JsonResponse({'error':'Can fetch agents at the moment, coordinate with administration'}, status=500)

class UpdateCrmApiDetails(LoginRequiredMixin,APIView):

	def post(self,request):
		print("starting_step",request.data)
		api_data = json.loads(request.POST.get('customer_raw_data',{}))
		original_dict = json.loads(request.POST.get('customer_raw_data',{}))
		contact_id = request.POST.get('contact_id','')
		created_date = request.POST.get('created_date','')
		error_dict = {}
		section_error_keys = []
		visit = ''
		default_crm_section_keys = ['customer_details', 'loan_details', 'address_details', 'asset_details','appointment_details','other_details']
		if api_data:
			section_keys  = list(api_data.keys())
			for key in section_keys:
				if key in default_crm_section_keys:
					section_error_keys.append(key)
			if section_error_keys:
				error_dict['error'] = ",".join(section_error_keys)+" is not defined in the CRM Fields"
			#add appointment dict here and update the details 
			api_data['other_details'].update(SBFC_STATIC_CRM_DATA['other_details'])
			api_data['geo_cordinate'] = SBFC_STATIC_CRM_DATA['geo_cordinate']
			# api_data.update(SBFC_STATIC_CRM_DATA)

			if created_date:
				api_data['other_details']['created_dt'] = created_date
			if 'lead_source' not in api_data['other_details']:
				api_data['other_details']['lead_source'] = ''
			if 'filler1' not in api_data['other_details']:
				api_data['other_details']['filler1'] = ''

			if 'address_details' in api_data:
				if 'area' not in api_data['address_details']:
					api_data['address_details']['area'] = ''
				else:
					api_data['address_details']['area'] = ''
			if 'mmi_address' in api_data['address_details']: 
				del api_data['address_details']['mmi_address']
			if 'visit' in api_data['address_details']:
				visit = api_data['address_details']['visit']
				del api_data['address_details']['visit']
			if 'transactional_address' in api_data['address_details']:
				del api_data['address_details']['transactional_address']
			print("sending this dict to SBFC",api_data)
			try:
				if api_data['other_details']['employee_code'] != '':
					api_data['other_details']['filler2'] = api_data['other_details']['employee_code']
					api_data['other_details']['filler3'] = api_data['other_details']['employee_name']
					api_data['other_details']['filler4'] = api_data['other_details']['employee_role']
					del api_data['other_details']['employee_code']
					del api_data['other_details']['employee_name']
					del api_data['other_details']['employee_role']
			except Exception as e:
				print('deleted already 2',str(e))
			print("final response sending", api_data)
			response = requests.post(url=settings.SBFC_CRM_DETAILS_PUSH_URL, headers=settings.SBFC_CRM_CREDENTIALS, json=api_data)
			if response.status_code == 200:
				response_text = json.loads(response.text)
				if response_text['message'] == "Success":
					if contact_id!='0':
						contact_inst = Contact.objects.filter(id=contact_id)[0]
						if 'appointment_details' in contact_inst.customer_raw_data and 'type' in contact_inst.customer_raw_data['appointment_details'] and 'type' in api_data['appointment_details']:
							contact_inst.customer_raw_data['appointment_details']['type'] = api_data['appointment_details']['type']
							contact_inst.save()
					else:
						print("Could Not Update the Status to Update, Due to Contact Id is not Present")
					print('data pushed successfully to sbfc api', response_text)
					if api_data['appointment_details']['type']=='unconfirm':
						return JsonResponse({'success':'mail not sent contact saved'})
					else:
						response = AppointmentDetailsEmailApi(original_dict,visit,request.user)
						if 'success' in response:
							message = str(response_text["message"]) +"/"+ str(response['success'])
							return JsonResponse({"success":message,'response_data':response_text["code"]})
						else:
							message = str(response_text["message"]) +"/"+ str(response['error'])
							return JsonResponse({"warning":message,'response_data':response_text["code"]})
				else:
					print('error while pushing data to sbfc api', response_text)
					return JsonResponse({"error":response_text["message"],"status":response_text["code"]})
			else:
				print('unable to call sbfc push api', response.status_code)
				return JsonResponse({"error":"Can't Call the CRM API", "status":response.status_code})
		else:
			print('API Data is empty')
			return JsonResponse({'error':"API Data is empty ", "status":404})
...
class CreateEditHolidaysApiView(LoginRequiredMixin,APIView):
	...
	def get(self,request,pk="",**kwargs):
		...
		if pk:
			...
			context['hol_camps'] = list(Holidays.objects.filter(id=pk).values_list('campaigns__name',flat=True))
			if permission_dict['can_update']:
				can_view = True
		else:
			if permission_dict['can_create']:
				can_view = True
			holidays = ""
		context = {**context,"request": request,"holiday_list_status": Status, "audio_files":audio_files,"holidays":holidays,
			'can_view':can_view,"Except_mode": Except,'contact_fields':contact_fields,'is_edit':is_edit}
		context['all_campaigns']=Campaign.objects.all()
		context['can_switch'] = permission_dict['can_switch']
		context['can_boot'] = permission_dict['can_boot']
		#"assign_campaigns":str
		return Response(context)
	def post(self,request,pk=None,**kwargs):
		if pk:
		...
		if holiday_file.size <= 1:
			...
			if holiday_file.name.endswith('.csv'):
				...
				data = pd.read_excel(holiday_file, encoding = "unicode_escape",parse_dates=['holiday_date'],
						converters={'name':str,'description':str,'status':str,"assign_campaigns":"str"})
			user_columns = ['name', 'holiday_date','description','status','assign_campaigns']
			column_names = data.columns.tolist()
			valid = all(elem in column_names for elem in user_columns)
			if valid:
...
def AppointmentDetailsEmailApi(data,visit,user):
	if data:
		middle_name = ''
		if data['customer_details']['middle_name']:
			middle_name = data['customer_details']['middle_name']
		customer_name = str(data['customer_details']['first_name'])+' '+str(middle_name)+' '+str(data['customer_details']['last_name'])
		customer_mobile = data['customer_details']['mobile_no']
		address_data = ''
		if data['address_details']['address_line1']:
			address_data = data['address_details']['address_line1']
		if data['address_details']['address_line2']:
			address_data = address_data + data['address_details']['address_line2']
		if data['address_details']['landmark']:
			address_data = address_data + data['address_details']['landmark']
		if 'transactional_address' in data['address_details']:
			pass
		else:
			data['address_details'].update({'transactional_address': None})
		customer_address = str(address_data)+" "+ str(data['address_details']['pincode']) +" "+ str(data['address_details']['city']) +" "+ str(data['address_details']['state']) +" "+ str(data['address_details']['transactional_address'])
		assigned_by = user.username 
		if 'appointment_details' in data and 'agent_details' in data['appointment_details']:
			agent_names = [agents['agent_name'] for agents in data['appointment_details']['agent_details']]
			agents_mail_sent = [] 
			for agent in data['appointment_details']['agent_details']:
				other_agents = ','.join(list(filter(lambda agent_names:  agent['agent_name'] != agent_names, agent_names)))
				email_data = {
				   "Agent_Id":agent['agent_cd'],
				   "DateTime":agent['schedule_time_to'],
				   "Agent_Name":agent['agent_name'],
				   "Customer_Name": customer_name,
				   "Customer_Mobile": str(customer_mobile),
				   "Customer_Address": customer_address.strip(),
				   "2nd_Agent_Name":other_agents,
				   "Visit":visit,
				   "Assigned_By":assigned_by,
				   "Type": data['appointment_details']['type']
				}
				print("Data for Sending email",email_data)
				response = requests.get(url=settings.SBFC_AGENT_SCHEDULE_APPOINTMENT_MAIL, headers=email_data)
				if response.status_code == 200:
					return_data = json.loads(response.text)
					response_text = json.loads(return_data)
					if response_text['RESPONSE']['STATUS'] == 'S':
						print("Response from the SBFC",response_text)
						agents_mail_sent.append(agent['agent_name'])
						# return JsonResponse({'success':response_text['RESPONSE']['MESSAGE']})
					else:
						return {"error":"STATUS is not responded as Success for"}
				else:
					return {"error":"Unable to connect to the server"}
			if agents_mail_sent:
				return {"success":f"MAIL SENT SUCCESSFULLY TO {','.join(agents_mail_sent)} and Appointement Successfully Confirmed."}
		else:
			return {"error":"Appointment Details or Agent Details are Not Found"}
	else:
		return {'error':"Data is Not availabile "}
...
# class AppointmentDetailsEmailApi(LoginRequiredMixin,APIView):
# 	""" Email sending to the field agents to which appointment is booked """
# 	login_url = '/'
# 	def post(self,request):
# 		data = json.loads(request.POST.get('customer_raw_data',{}))
# 		if data:
# 			customer_name = data['customer_details']['first_name']+' '+data['customer_details']['middle_name']+' '+data['customer_details']['last_name']
# 			customer_mobile = data['customer_details']['mobile_no']
# 			customer_address = data['address_details']['address_line1']+" "+data['address_details']['address_line2'] +" "+ data['address_details']['landmark']+" "+ data['address_details']['pincode'] +" "+ data['address_details']['city'] +" "+ data['address_details']['state'] +" "+ data['address_details']['transactional_address']
# 			assigned_by = request.user.username 
# 			visit = data['address_details']['visit']
# 			if 'appointment_details' in data and 'agent_details' in data['appointment_details']:
# 				agent_names = [agents['agent_name'] for agents in data['appointment_details']['agent_details']]
# 				agents_mail_sent = [] 
# 				for agent in data['appointment_details']['agent_details']:
...
# 							# return JsonResponse({'success':response_text['RESPONSE']['MESSAGE']})
# 						else:
# 							return JsonResponse({"error":"STATUS is not responded as Success for"})
# 					else:
# 						return JsonResponse({"error":"Unable to connect to the server"})
# 				if agents_mail_sent:
# 					return JsonResponse({"success":f"MAIL SENT SUCCESSFULLY TO {','.join(agents_mail_sent)}"})
# 			else:
# 				return JsonResponse({"error":"Appointment Details or Agent Details are Not Found"})
# 		else:
# 			return JsonResponse({'error':"Data is Not availabile "})


class VoiceBlasterCreateTTS(APIView):
	"""
	This will create a TTS File on the fly once call answered by the customer.
	"""
	permission_classes = [AllowAny]
	def get(self,request,pk):
		contact_id = int(request.GET.get("contact_id",pk))
		contact = Contact.objects.filter(id=contact_id).first()
		voice_blaster_tts_check(contact)
		return JsonResponse({"success":f"{contact.id} audio is created"})
