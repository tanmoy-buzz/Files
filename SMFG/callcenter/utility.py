...
def pd_to_csv(df,download_report_id,report_name, user, file_type='csv'):
	try:
		...
		if file_type == 'csv':
			...
			print(file_path,'pathhhhhhhhhhh for apr report')
		...
		if download_report_id !=None:
			print(download_report_id,'--------------repo id----')
...
def download_call_detail_report(filters, user, col_list, serializer_class, download_report_id=None):
	...
	try:
		...
		if selected_campaign:
			sql_query_where = ''' ((callcenter_calldetail.campaign_name IN {campaigns} AND COALESCE(callcenter_calldetail.user_id, 0) IN {users}) OR (callcenter_calldetail.campaign_name IN {campaigns} AND callcenter_calldetail.user_id IS NULL))
			'''.format(campaigns=str(tuple(selected_campaign)), users=str(tuple(all_users)))
		else:
			sql_query_where = '''((COALESCE(callcenter_calldetail.user_id, 0) IN {users}) OR (callcenter_calldetail.user_id IS NULL))'''.format(users=str(tuple(all_users)))
		if selected_user and selected_campaign:
			...
			if selected_campaign:
				...
				if not (user.is_superuser or admin):
					...
					sql_query_where = '''((callcenter_calldetail.campaign_name IN {campaigns} AND COALESCE(callcenter_calldetail.user_id, 0) IN {camp_users}) OR (callcenter_calldetail.campaign_name IN {campaigns} AND callcenter_calldetail.user_id IS NULL))'''.format(campaigns=str(tuple(selected_campaign)), camp_users=str(tuple(get_camp_users)))
		...
		print(file_path, 'path locationisssssssss')
		f = open(file_path, 'rb')
		download_report = DownloadReports.objects.get(id=download_report_id)
		print(download_report,'reportid=============')
		download_report.downloaded_file.save(os.path.basename(f.name), File(f), save=True)
		print(os.path.basename(f.name),'file_SAVEDDDDD',File(f),'saved full name')
		download_report.is_start = False
		download_report.save()
		f.close()
#		os.remove(file_path)
...
def action_agent_performance_report(filters,download=False,download_report_id=0):	# try:
		all_users = filters.get("all_users", [])
		print('start_-------------')
		selected_campaign = filters.get("selected_campaign", [])
		start_date = filters.get("start_date", "")
		download_type = "csv"
		selected_user = filters.get('selected_user',"")
		print(selected_campaign, "====filtercampaign=====")
		...
		print('nocamp  passs')
		if selected_campaign:
			print(selected_campaign,'chosen camppp====')
		...
		if len(apr_main):
			...
			print(breaks_activity_df,"====11111===")
			...
			try:
				...
				print(required_agents_ids, "===33333====")
