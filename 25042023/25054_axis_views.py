...
def user_hierarchy_func(user_id,reporting_users=""):
	...
	try:
		...
		elif user_obj.user_role.access_level == 'Admin': 
			users_list = User.objects.exclude(is_superuser=True).values_list("id",flat=True)
		else:
			created_users = list(User.objects.filter(created_by__id=user_id).values_list('id',flat=True))
			reporting_users=[user_id]
			print('11111111111',reporting_users)
			users_list = reporting_users+created_users
			while len(reporting_users) > 0:
				all_users=list(User.objects.filter(reporting_to__id__in=reporting_users).values_list('id',flat=True))
				users_list=users_list+all_users
				reporting_users=all_users
		users_list=list(set(users_list))#remove duplicates
		print('2222222222',users_list)
		return users_list
	except Exception as e:
		print('exception in user_hierarchy_func',e)

def old_user_hierarchy_func(user_id,users_list=""):
	"""this function takes the username and returns usernames based on reporting_to hierarchy"""
	user_obj = User.objects.get(id=user_id)
	if user_obj.is_superuser:
		users_list = list(User.objects.all().values_list("id",flat=True))
	elif str(user_obj.user_role) == 'admin' or str(user_obj.user_role and user_obj.user_role.access_level) == 'Admin':
