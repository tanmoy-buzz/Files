...
class CallDetailReportView(LoginRequiredMixin,APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		all_fields = {'diallereventlog':['hangup_cause','hangup_cause_code'],'smslog':['sms_sent','sms_message'],
		...
		'bill_sec','ivr_duration','call_duration','feedback_time','call_length','hangup_source','internal_tc_number','external_tc_number','progressive_time','preview_time','predictive_wait_time','inbound_wait_time','blended_wait_time','dialed_status'],
...
class CallRecordingView(LoginRequiredMixin, generics.ListAPIView):
	...
	def post(self,request):
		...
		if start_date:
			queryset = DiallerEventLog.objects.filter(created__gte=start_date,dialed_status__in=['Connected','Voice blaster with talk']).select_related("campaign", "user")
			if end_date:
				queryset = queryset.filter(created__lte=end_date)
		else:
			queryset = DiallerEventLog.objects.filter(dialed_status__in=['Connected','Voice blaster with talk']).select_related("campaign", "user")
...
class DiallerLogin(LoginRequiredMixin, APIView):
	...
	def post(self, request):
		...
		if 'thirdparty_data_push' in campaign.dial_method and  campaign.dial_method['thirdparty_data_push']:#checking gold genie
			try:
	...
	def post(self,request):
		...
		if callback_filter_date:
			...
			campaign_ac_obj = CurrentCallBack.objects.filter(Q(user=user_obj.extension,campaign = campaign)
						|Q(callback_type='queue',campaign=campaign, user=None),Q(schedule_time__date=callback_date)).exclude(callmode="inbound", user=None).exclude(status='Locked').order_by('-schedule_time')
		else:
			campaign_ac_obj = CurrentCallBack.objects.filter(Q(user=user_obj.extension,campaign = campaign)
...
class GetCallHistoryApiView(LoginRequiredMixin, APIView):
	...
	def post(self, request, format=None):
		...
		paginate_by = request.POST.get('selected_page',10)
		if unique_id:	
			if contact_id:
				call_history = CallDetail.objects.filter(Q(contact_id=contact_id)|Q(uniqueid=unique_id))
			else:
				call_history = CallDetail.objects.filter(Q(customer_cid=dialed_no, campaign_name=campaign)|Q(uniqueid=unique_id))
		else:
			if contact_id:
				call_history = CallDetail.objects.filter(contact_id=contact_id)
			else:
				call_history = CallDetail.objects.filter(customer_cid=dialed_no, campaign_name=campaign)
		paginate_obj = get_paginated_object(call_history, page, paginate_by)
		history_data = CallHistorySerializer(paginate_obj,many=True).data
		return Response({'total_records': paginate_obj.paginator.count, 'total_pages': paginate_obj.paginator.num_pages, 'page': paginate_obj.number,'has_next': paginate_obj.has_next(),'has_prev': paginate_obj.has_previous(),
			'start_index':paginate_obj.start_index(), 'end_index':paginate_obj.end_index(),
...
class SmsTemplateCreateEditApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request, pk="", **kwargs):
		...
		if check_non_admin_user(request.user):
			campaigns = campaigns.filter(Q(users=request.user)|Q(group__in=request.user.group.all())|Q(created_by=request.user)).distinct()
		permission_dict = kwargs['permissions']
		contact_fields = ['numeric', 'first_name', 'last_name', 'email','dialed_uuid']
...
class SmsTemplateGetCrmFieldsApiView(APIView):
	...
	def post(self, request, **kwargs):
		...
		crm_fields = get_customizable_crm_fields(campaign_name)
		# contact_fields = ['numeric', 'alt_numeric', 'first_name', 'last_name', 'email']
		contact_fields = ['numeric', 'alt_numeric', 'first_name', 'last_name', 'email','dialed_uuid']
		contact_fields = contact_fields + crm_fields
		context = {'contact_fields':contact_fields}
		return JsonResponse(context)
...
class SendSMSApiView(APIView):
	...
	def post(self,request):
		...
		message=SMSTemplate.objects.filter(Q(campaign_id=campaign_id)|Q(template_type=0)).values('id','text')
...
class VoiceBlasterCreateAPIView(LoginRequiredMixin, APIView):
	...
	def get(self,request,pk=None, **kwargs):
		...
		context['campaign']=Campaign.objects.all().values('name','id')
		context['api_status'] = Status
		context['api_modes'] = API_MODE
		context['user_fields'] = ['user_id','username','user_extension','campaign_id', 'campaign_name']
	...
	def get(self,request,pk=None, **kwargs):
		...
		if vb_obj.campaign.exists():
			if crm_obj:
				...
				#  = get_crm_fields_dict(vb_obj.campaign.first().name)
			else:
				default_cols = ['numeric','alt_numeric','first_name','last_name','email','priority']
				camp_crm_fields = []
...
class CreateEditHolidaysApiView(LoginRequiredMixin,APIView):
	...
	def get(self,request,pk="",**kwargs):
		...
		if pk:
			...
			context['hol_camps'] = list(Holidays.objects.filter(id=pk).values_list('campaigns__name',flat=True)) #optimize
			if permission_dict['can_update']:
				can_view = True
		else:
			if permission_dict['can_create']:
	...
	def post(self, request):
		...
		if holiday_file.size <= 1:
			...
			if holiday_file.name.endswith('.csv'):
				data = pd.read_csv(holiday_file, na_filter=False, encoding = "unicode_escape", escapechar='\\', dtype={"name" : "str",'description':'str',"status":"str","assign_campaigns":"str"},parse_dates=['holiday_date'],dayfirst=True)
			else:
...
def AppointmentDetailsEmailApi(data,visit,user):
	if data:
		...
		if 'appointment_details' in data and 'agent_details' in data['appointment_details']:
			...
			if agents_mail_sent:
				return {"success":f"MAIL SENT SUCCESSFULLY TO {','.join(agents_mail_sent)}"}
		else:
