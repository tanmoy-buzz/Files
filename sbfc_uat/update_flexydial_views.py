...
def getCRMdata(contact_id,reciever,campaign_name,primary_dispo,contact_obj,user_obj,msg,sms_data=''):
	try:
		if campaign_name:
			...
			if len(contact_obj)!=0 and Contact_objs_duplicates.exists() and primary_dispo:
				...
				for dat in sms_data.items():
					pass
				all_crm_fields_dict.update({"${UserId}":str(user_obj),"${user_first_name}":user_obj.first_name if user_obj.first_name else "","${user_last_name}":user_obj.last_name if user_obj.last_name else "","${user_email}":user_obj.email if user_obj.email else "","${first_name}":contact_objs_duplicates.first().first_name,"${last_name}":contact_objs_duplicates.first().last_name,"${numeric}":contact_objs_duplicates.first().numeric,"${email}":contact_objs_duplicates.first().email,"${dialed_uuid}":sms_data.get('session_uuid')})
				for replace_string in all_crm_fields_dict:
					if all_crm_fields_dict[replace_string]!=None:#this condition needed if it's uploded without crm data
						aa= all_crm_fields_dict.get(replace_string)
						msg=msg.replace(replace_string,str(all_crm_fields_dict[replace_string]),999)
				...
				if contact_obj.first().phonebook_id:
					...
					for dat in sms_data.items():
						pass
					all_crm_fields_dict.update({"${UserId}":str(user_obj),"${user_first_name}":user_obj.first_name if user_obj.first_name else "","${user_last_name}":user_obj.last_name if user_obj.last_name else "","${user_email}":user_obj.email if user_obj.email else "","${first_name}":contact_obj.first().first_name,"${last_name}":contact_obj.first().last_name,"${numeric}":contact_obj.first().numeric,"${email}":contact_obj.first().email,"${dialed_uuid}":sms_data.get('dialed_uuid')})
					for replace_string in all_crm_fields_dict:
						if all_crm_fields_dict[replace_string]!=None:#this condition needed if it's uploded without crm data
							aa= all_crm_fields_dict.get(replace_string)
							msg=msg.replace(replace_string,all_crm_fields_dict[replace_string],999)
				else:
					all_crm_fields_dict={"${UserId}":str(user_obj),"${user_first_name}":user_obj.first_name if user_obj.first_name else "","${user_last_name}":user_obj.last_name if user_obj.last_name else "","${user_email}":user_obj.email if user_obj.email else "","${first_name}":contact_obj.first().first_name,"${last_name}":contact_obj.first().last_name,"${numeric}":contact_obj.first().numeric,"${email}":contact_obj.first().email}
					for replace_string in all_crm_fields_dict:
						if all_crm_fields_dict[replace_string]!=None:#this condition needed if it's uploded without crm data
							msg=msg.replace(replace_string,all_crm_fields_dict[replace_string],999)
					return msg
...
def sendSMS(data,template_id,primary_dispo,user_obj,camp_obj,contact_obj):
	...
	if contact_id=='' or contact_id==None:
		contact_id=0
	else:
		contact_id=int(contact_id)
	msg=getCRMdata(contact_id,reciever,campaign_name,primary_dispo,contact_obj,user_obj,msg,data)
