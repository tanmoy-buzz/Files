...
from Zoho.serializers import ClickToCallUserConfigSerializer
...
class LogoutAPIView(LoginRequiredMixin, APIView):
	...
	def get(self,request,makeInactive=''):
		...
		if makeInactive:
			...
			request.user.save()
		try:
			last_activity = AgentActivity.objects.filter(user=request.user).first()
			if last_activity and last_activity.event != "LOGOUT":
				time.sleep(0.1)
				last_activity = AgentActivity.objects.filter(user=request.user).first()
				if last_activity and last_activity.event != "LOGOUT":
					...
					AgentActivity(**{'user':request.user,'event':'LOGOUT','event_time':now_date,'tos':time_diff,'idle_time':time_diff}).save()
		except Exception as e:
			print("error in logout due to agent activity",e)
...
class UsersCreateApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request):
		...
		enable_wfh = True#pickle.loads(settings.R_SERVER.get('enable_wfh') or pickle.dumps(False))
		switch_list = Switch.objects.filter().values("id","name")
		gsm_trunk_list = DialTrunk.objects.filter(status="Active").values('id','name','did_range','dial_string')
		...
		context = {**user_variables_default_values,'available_users': users,'request': request,
				...
				'call_type':CALL_TYPE,"gsm_trunk_list":gsm_trunk_list
				}
		return Response(context)
	...
	def post(self, request):
			...
			if user_serializer.is_valid():
				if user_variable_serializer.is_valid():
					...
					if request.POST['call_type'] == '4':
						print(request.POST,'oooooooooooooo')
						c2c_user_config_ser = ClickToCallUserConfigSerializer(data = request.data)

						if c2c_user_config_ser.is_valid():
							c2c_user_config_ser.save(config_user= user,created_by=request.user)
						else:
							print({"errors": c2c_user_config_ser.errors})
...
class UsersEditApiView(LoginRequiredMixin, APIView):
	...
	def get(self, request, pk, format=None,**kwargs):
		...
		context = {'request': request, 'user_status': UserStatus, 'groups': groups,
				...
				'enable_wfh': enable_wfh,"switch_list":switch_list, 'trunk_list':trunk_list, "gsm_trunk_list":gsm_trunk_list, 'used_did_list':list(used_did_list)}
	...
	def put(self, request, pk, format=None):
		...
		user_variable_serializer = UserVariableSerializer(user.properties, data=request.data)
		if serializer.is_valid():
			serializer.save()
			if user_variable_serializer.is_valid():
			...
			if request.POST['call_type'] == '4':
				try:
					c2c_config = user.c2c_config_user
					c2c_user_config_ser = ClickToCallUserConfigSerializer(user.c2c_config_user ,data = request.data)
				except:
					c2c_config = None
					c2c_user_config_ser = ClickToCallUserConfigSerializer(data = request.data)
					
				if c2c_user_config_ser.is_valid():
					c2c_user_config_ser.save(created_by=request.user,config_user=user)
				else:
					print({"errors": c2c_user_config_ser.errors})
...
class CallDetailReportView(LoginRequiredMixin,APIView):
	...
	def post(self,request, *args, **kwargs):
		...
		print(len(queryset),'querysetqueryset')
		print('QUERYYYYY %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%',str(queryset.query))
...
class CallRecordingView(LoginRequiredMixin, generics.ListAPIView):
	...
	def get(self, request, *args, **kwargs):
		...
		if request.user.is_superuser:
			...
			user_list = User.objects.all().exclude(is_superuser=True).prefetch_related(
					'group', 'reporting_to', 'user_role').values("id", "username")
		else:
...
class GetTotalCallsPerDay(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		...
		durations = [timedelta(hours=t.hour, minutes=t.minute, seconds=t.second) for t in list(total_callsperday_obj.values_list('call_duration',flat=True))]
		call_length['total_call_length'] =  str(sum(durations, timedelta()))
		...
		return Response({'total_records': paginate_obj.paginator.count,'total_pages': paginate_obj.paginator.num_pages,
				...
				'total_callperday':agent_callperday[::-1],'call_length':call_length})
...
class GetTotalCallsPerMonth(LoginRequiredMixin,APIView):
	...
	def post(self,request):
		...
		durations = [timedelta(hours=t.hour, minutes=t.minute, seconds=t.second) for t in list(queryset.values_list('call_duration',flat=True))]
		call_length['total_call_length'] =  str(sum(durations, timedelta()))#call_length_detail['call_duration']
		...
		return Response({'total_records': paginate_obj.paginator.count,'total_pages': paginate_obj.paginator.num_pages,
				'page': paginate_obj.number,'has_next': paginate_obj.has_next(),'has_prev': paginate_obj.has_previous(),
				'start_index':paginate_obj.start_index(), 'end_index':paginate_obj.end_index(),
				'total_data':agent_callpermonth[::-1],'call_length':call_length})
...
class GetCallHistoryApiView(LoginRequiredMixin, APIView):
	...
	def post(self, request, format=None):
		...
		unique_id = request.POST.get("unique_id",'')
		print('Unique ID :',unique_id, 'Contact id :',contact_id,'Campaign :',campaign, 'Dialed Number :',dialed_no)
		if unique_id:
			if contact_id:
...
class BillingView(LoginRequiredMixin,APIView):
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			print('Exception in Billing Report View ', e)
		return JsonResponse({'total_records': queryset.paginator.count, 'total_pages': queryset.paginator.num_pages,
				'page': queryset.number, 'has_next': queryset.has_next(), 'has_prev': queryset.has_previous(),
				'start_index':queryset.start_index(), 'end_index':queryset.end_index(),
				'table_data':result})
