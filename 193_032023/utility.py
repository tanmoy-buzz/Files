...
from django.db.models import Q, F, Sum, TimeField, Value, Count, Prefetch, F, TextField
...
import itertools
from .models import (CampaignGroup,CampaignSchedule, Switch, DialTrunk, User, Group,
	...
	EmailGateway,CampaignVariable, SkilledRoutingCallerid,Daemons,Holidays)
from flexydial.constants import (Status,CALL_MODE, CAMPAIGN_STRATEGY_CHOICES, DIAL_RATIO_CHOICES, CDR_DOWNLOAD_COl,QC_FEEDBACK_COL)
from .serializers import (CampaignPaginationSerializer,GroupSerializer,CallDetailSerializer, CallBackContactSerializer,
...
def user_hierarchy(request,camp_name):
	...
	if request.user.is_superuser:
		team_extensions = UserVariable.objects.filter(user__in=users).exclude(user__user_role__access_level__in=['Admin','Manager',]).values_list("extension", flat=True)
	elif admin:
		team_extensions = UserVariable.objects.filter(user__in=users.exclude(is_superuser=True)).exclude(user__user_role__access_level__in=['Admin','Manager',]).values_list("extension", flat=True)
	else:
		camp_users = []
...
def upload_dnc_nums(data, logged_in_user):
	...
	try:
		...
		print("error for upload dnc nums",e)
	finally:
		transaction.commit()
...
def get_campaign_mis(data, request) :
	...
	all_campaigns = all_campaigns.split(',')
	selected_campaign = data.getlist("selected_campaign", [])	
	selected_campaigngroup = data.getlist("selected_campaigngroup", [])
	...
	if selected_campaigngroup:	
		camp_grps = CampaignGroup.objects.filter(name__in=selected_campaigngroup)	
		for camp_grp in camp_grps:	
			selected_campaign += [i.strip() for i in camp_grp.campaign_name.split(',')]	
...
def save_email_log(report_name,from_email="",to_email="",response="",user=""):
	...
	try:
		...
		print("error for save email log",e)
	finally:
		transaction.commit()
		connections['default'].close()
...
def save_download_log(user, filename):
	...
	try:
		...
		print("error for save download log",e)
	finally:
		transaction.commit()
		connections["default"].close()
...
def action_agent_performance_report(filters,logged_in_user,download=False,download_report_id=0):
	try:
		all_users = filters.get("all_users", [])
		selected_campaign = filters.get("selected_campaign", [])
		download_type = "csv"
		...
		all_fields =  set(Campaign.objects.filter(status='Active').exclude(disposition=None).values_list("disposition__name", flat=True))
		wfh_dispo_list = list(Campaign.objects.filter(wfh=True, status='Active').values_list('wfh_dispo',flat=True))
		disp_names_list = list(Disposition.objects.filter(status='Active').values_list('name',flat=True))
		
		wfh_dispo = set([ d for m in wfh_dispo_list for d in m.values()])
		all_fields.update(wfh_dispo)
		all_fields = sorted(all_fields)
		tmp_list = ["Total Dispo Count", "AutoFeedback", "AbandonedCall",
		'NC','Invalid Number',"RedialCount", "AlternateDial", "PrimaryDial", "NF(No Feedback)"]
		all_fields = tmp_list + list(all_fields)
		
		all_col = ['total_calls','total_login_time','app_login_time','app_idle_time','dialer_idle_time','pause_progressive_time','progressive_time','preview_time','inbound_wait_time',
                'blended_wait_time','hold_time','media_time','pause','pause_avg','wait_time','wait_time_avg','ring','ring_avg','talk','talk_avg','wrapup','wrapup_avg','customer','customer_avg','call_duration',
        ] + pause_breaks + ['dialer_login_time','first_login_time','last_logout_time','total_connected_calls','total_unique_connected_calls']+ all_fields
		...
		agent_activity_query = f'''select 
				...
				sum(predictive_wait_time) as wait_time,
		...
		if len(queryset) == 0:
			if download:
				return pd.DataFrame(columns=['username','full_name','supervisor']+all_col)
		...
		calldetail_query = f'''select user_id,campaign_name,
						...
						sum(ring_duration) as ring_calldetail,
						sum(call_duration) as call_duration,
						sum(bill_sec) as bill_sec,
						sum(feedback_time) as wrapup
						from callcenter_calldetail 
						where created>'{start_date}' and created < '{end_date}'
						group by user_id,campaign_name'''

		breaks_activity_query = f'''select user_id,sum(break_time) as pause,
						break_type 
						from callcenter_agentactivity where created>'{start_date}' and created < '{end_date}'
						group by user_id,break_type '''

		ring_duration_query = f'''select user_id,campaign_name,sum(ring_duration) as ring,sum(bill_sec) as customer_dialler from callcenter_diallereventlog where created>'{start_date}' and created < '{end_date}' group by user_id,campaign_name'''

		total_connected_calls_query= f''' select user_id,campaign_name,count(dialed_status) as total_connected_calls from callcenter_calldetail where dialed_status='Connected' AND  created>'{start_date}' and created < '{end_date}' group by user_id,campaign_name'''
		if download:
		...
		user_df['full_name'] =  user_df['first_name']+ ' '+ user_df['last_name'] #user_df[['first_name','last_name']].apply(lambda x:' '.join(x.map(str)), axis=1)
		# user_df['full_name'] = user_df['full_name'].str.replace(' ', '_')
		...
		aaa = f'''select id as cdr_id,user_id from callcenter_calldetail where created>'{start_date}' and created < '{end_date}' group by id,user_id'''
		
		df_aaa = pd.read_sql(aaa,conn)
		all_cdr_ids = df_aaa['cdr_id'].tolist()
		aaa = ','.join(map(str, all_cdr_ids))
		if not aaa:
			aaa = '0,1,2,3'
			cdr_query = f"SELECT * FROM callcenter_cdrfeedbck WHERE calldetail_id IN ({aaa})" #','.join(map(str, all_cdr_ids))
			df_cdr_query = pd.read_sql(cdr_query,conn)
		else:
			cdr_query = f"SELECT * FROM callcenter_cdrfeedbck WHERE calldetail_id IN ({','.join(map(str, all_cdr_ids))})"
			df_cdr_query = pd.read_sql(cdr_query,conn)

		whole_cdr_fb = pd.merge(df_aaa,df_cdr_query, how='left',right_on='calldetail_id', left_on='cdr_id')
		pivot_df = whole_cdr_fb.pivot_table(index='user_id', columns='primary_dispo', values='id', aggfunc='count', fill_value=0)
		pivot_df.reset_index(inplace=True)
		pivot_df['user_id'] = pivot_df['user_id'].astype('int')
		pivot_df = pivot_df.rename(columns = {'user_id':'dispo_user_id'})
		disp_names_list += ["AutoFeedback", "AbandonedCall",'NC','Invalid Number',"RedialCount", "AlternateDial", "PrimaryDial", "NF(No Feedback)"]
		for col in disp_names_list:
			if col not in pivot_df.columns:
				pivot_df[col] = 0

		total_dispo_counts = pivot_df[disp_names_list].sum(axis=1)
		pivot_df['Total Dispo Count'] = total_dispo_counts
		...
		all_time_related_cols = ['pause','pause_progressive_time','predictive_time',
						'progressive_time',
						'preview_time',
						'wait_time',
		...
		calldetail_df['ring'] = calldetail_df['ring'].fillna(calldetail_df['ring_calldetail'])
		calldetail_df['bill_sec'] = calldetail_df['customer_dialler'].fillna(calldetail_df['bill_sec'])
		calldetails_timecols = ['media_time','ring','hold_time','call_duration','bill_sec','wrapup']
		...
		calldetail_df_without_camp = calldetail_df.groupby('user_id').agg({
								...
								'calldetail_count':'sum','media_time':'sum','hold_time':'sum','ring':'sum','call_duration':'sum','bill_sec':'sum','wrapup':'sum'
							}).reset_index()
		del calldetail_df
		...
		if len(apr_main):
			...
			apr_main['talk_total'] = apr_main['bill_sec']+apr_main['wrapup']+apr_main['ring']
			apr_main['customer'] = apr_main['talk_total'] - apr_main['wrapup']
			apr_main['pause_avg'] = apr_main['pause']/apr_main['calldetail_count'] 
			apr_main['talk_avg'] = apr_main['talk_total']/apr_main['calldetail_count']
			apr_main['wrapup_avg'] = apr_main['wrapup']/apr_main['calldetail_count']
			apr_main['customer_avg'] = apr_main['customer']/apr_main['calldetail_count']
			apr_main['ring_avg'] =apr_main['ring']/apr_main['calldetail_count']
			apr_main['wait_time_avg'] = apr_main['wait_time']/apr_main['calldetail_count']

			columns_to_sum = ['pause_progressive_time', 'wait_time', 'preview_time', 'inbound_wait_time', 'blended_wait_time', 'dialer_app_idle_time', 'call_duration', 'wrapup', 'progressive_time']
			apr_main[columns_to_sum] = apr_main[columns_to_sum].fillna(0)

			apr_main['dialer_login_time'] = (apr_main[columns_to_sum].sum(axis=1)).astype(int)

			apr_main[['dialer_login_time']] = apr_main[['dialer_login_time']].fillna(0)
			
			# apr_main['dialer_login_time'] = apr_main['pause_progressive_time']+ apr_main['wait_time'] + apr_main['preview_time']+ apr_main['inbound_wait_time'] +apr_main['blended_wait_time']+ apr_main['dialer_app_idle_time']+ apr_main['call_duration']+ apr_main['wrapup']+ apr_main['progressive_time']

			apr_main['total_login_time'] = apr_main['dialer_login_time'] + apr_main['user_app_idle_time']+ apr_main['pause']
			...
			apr_main_with_fin_lout[['app_login_time']] = apr_main_with_fin_lout[['app_idle_time']]
			apr_main_final = apr_main_with_fin_lout[['app_idle_time','app_login_time','customer','customer_avg',
							'blended_time','blended_wait_time','pause','pause_avg',
							'call_duration','campaign','dialer_idle_time','dialer_login_time','wrapup',
							'wrapup_avg','first_login_time','hold_time','inbound_time',
							'inbound_wait_time','last_logout_time','media_time','pause_progressive_time',
							'predictive_time','wait_time','wait_time_avg','preview_time',
							'progressive_time','ring','ring_avg','talk_total','talk_avg','total_calls'
							,'total_login_time','total_unique_connected_calls','username','user_id','supervisor_name','full_name']]
			...
			all_breaks_df = breaks_activity_df.pivot_table('pause', 'user_id','break_type','sum')
			...
			final_dataframe = pd.merge(final_dataframe,pivot_df,how='left',left_on= 'user_id',right_on='dispo_user_id')
			...
			final_timecols = [
				'pause','pause_progressive_time','predictive_time','progressive_time','preview_time',
				'wait_time','inbound_time','blended_time','inbound_wait_time','blended_wait_time',
				'app_idle_time','dialer_idle_time','media_time','ring','hold_time','call_duration',
				'customer','wrapup','talk_total','pause_avg','talk_avg','wrapup_avg',
				'customer_avg','ring_avg','wait_time_avg','dialer_login_time','total_login_time','app_login_time'
				
			] #'customer_talk', ,'customer_talk',,'customer_talk'

			final_dataframe[['pause','pause_progressive_time','predictive_time','progressive_time','preview_time',
				'wait_time','inbound_time','blended_time','inbound_wait_time','blended_wait_time',
				'app_idle_time','dialer_idle_time','media_time','ring','hold_time','call_duration',
				'customer','wrapup','talk_total','pause_avg','talk_avg','wrapup_avg',
				'customer_avg','ring_avg','pwait_time_avg','dialer_login_time','total_login_time']] = final_dataframe[['pause','pause_progressive_time','predictive_time','progressive_time','preview_time',
				'wait_time','inbound_time','blended_time','inbound_wait_time','blended_wait_time',
				'app_idle_time','dialer_idle_time','media_time','ring','hold_time','call_duration',
				'customer','wrapup','talk_total','pause_avg','talk_avg','wrapup_avg',
				'customer_avg','ring_avg','wait_time_avg','dialer_login_time','total_login_time']].fillna('00:00:00')
			...
			# print(final_dataframe.columns,"FINALDATFRAME")
			...
			report_visibility_column=[]
			report_visibility_column=get_report_visible_column('3',logged_in_user)
			if report_visibility_column :
				final_dataframe = final_dataframe[report_visibility_column]
			if download:
		...
		if download:
			return final_dataframe #pd.DataFrame(columns=['username','full_name','supervisor']+all_col)
		return call_result,users
	except Exception as e:
...
def download_agent_perforance_report(filters, user, col_list, download_report_id):
	try:
		...
		call_result = action_agent_performance_report(filters,logged_in_user,True,download_report_id)
	...
	try:
		...
		for user in queryset:
			...
			calldetail_cal = calldetail.aggregate(media_time=Cast(Coalesce(Sum('media_time'),default_time),TextField()),
				ring=Cast(Coalesce(Sum('ring_duration'),default_time),TextField()),
				...
				customer=Cast(Coalesce(Sum('bill_sec'),default_time),TextField()),
				wrapup=Cast(Coalesce(Sum('feedback_time'),default_time),TextField()),
				)
			activity_cal = agentactivity.aggregate(pause=Cast(Coalesce(Sum('break_time'),default_time),TextField()),
				...
				wait_time=Cast(Coalesce(Sum('predictive_wait_time'),default_time),TextField()),
			...
			dispo_time = convert_into_timedelta(calldetail_cal['wrapup']).total_seconds()
			ring_time = convert_into_timedelta(calldetail_cal['ring']).total_seconds()
			pw_time = convert_into_timedelta(activity_cal['wait_time']).total_seconds()
			talk_total = int(talk_call) + int(dispo_time) +int(ring_time)
			customer_talk = int(talk_call) + int(ring_time)
			...
			if total_calls:
				break_time_avg = ((convert_into_timedelta(activity_cal['pause']).total_seconds())/total_calls)
				bill_sec_avg = ((int(talk_call)+int(dispo_time)+int(ring_time))/total_calls)
				feedback_time_avg = ((convert_into_timedelta(calldetail_cal['wrapup']).total_seconds())/total_calls)
				customer_avg_talk = ((int(talk_call)+int(ring_time))/total_calls)
				pw_wait_avg = (int(pw_time)/total_calls)
				# call_duration_avg = ((convert_into_timedelta(calldetail_cal['customer']).total_seconds())/total_calls)
				wait_avg = ((convert_into_timedelta(calldetail_cal['ring']).total_seconds())/total_calls)
				calldetail_cal['pause_avg'] = convert_timedelta_hrs(timedelta(seconds=break_time_avg)) 
				calldetail_cal['talk_avg'] = convert_timedelta_hrs(timedelta(seconds=bill_sec_avg)) 
				calldetail_cal['wrapup_avg'] =  convert_timedelta_hrs(timedelta(seconds=feedback_time_avg)) 
				...
				activity_cal['wait_time_avg'] = convert_timedelta_hrs(timedelta(seconds=pw_wait_avg)) 
			else:
			...
			dialer_login_time = int(pp_time)+int(pw_time)+int(preview_time)+int(iw_time)+int(bw_time)+int(
				di_time)+int(call_duration)+int(dispo_time)+int(p_time)
			# total_login_time = int(dialer_login_time)+int(ai_time)+int(break_time) +int(ring_time)
			...
			calldetail_cal['dialer_login_time'] = convert_timedelta_hrs(timedelta(seconds=dialer_login_time)) 
			calldetail_cal['total_login_time'] = convert_timedelta_hrs(timedelta(seconds=total_login_time)) 
			calldetail_cal = {**calldetail_cal, **activity_cal ,**break_time_cal}
			calldetail_cal['total_calls'] = total_calls 
			...
			calldetail_cal["AbandonedCall"] =  calldetail.filter(user_id=user,dialed_status="Abandonedcall",cdrfeedback__primary_dispo='').count()
			calldetail_cal["NC"] = calldetail.filter(user_id=user,dialed_status="NC",cdrfeedback__primary_dispo='').count()
			calldetail_cal["Invalid Number"] = calldetail.filter(user_id=user,dialed_status="Invalid Number",cdrfeedback__primary_dispo='').count()
...
def download_management_performance_report(filters, user, col_list, download_report_id):
	...
	try:
		...
		for date , user in itertools.product(date_range, queryset):
			qr_end_date = date + timedelta(days=1)
			...
			user=User.objects.filter(id=user.id).prefetch_related(Prefetch('adminlogentry_set',queryset=AdminLogEntry.objects.filter(created__gte=date.date(),created__lte=qr_end_date.date()))).first()
			...
			login = management_log.filter(created__gte=date.date(),created__lte = qr_end_date.date(),event_type='LOGIN')

			logout = management_log.filter(created__gte=date.date(),created__lte = qr_end_date.date(),event_type='LOGOUT')
...
def download_agent_mis(filters, user, col_list, download_report_id):
	...
	try:
		...
		for date, user in itertools.product(date_range, users):
			...
			call_details = CallDetail.objects.filter(created__gte=date.date(),created__lte=end_date.date()).filter(Q(campaign_name__in=all_campaigns, user=user)| Q(user=user))
...
def download_pendingcontacts_report(filters, user, col_list, download_report_id):
	try:
		...
		queryset = Contact.objects.filter(Q(campaign__in=filter_by_campaign), Q(user=None)| Q(user=''),Q(created_date__lte
												   =datetime.today()),Q(status="NotDialed"))
...
def download_phonebookinfo_report(filters, user, col_list, download_report_id):
	...
	try:
		...
		transaction.commit()
		connections["default"].close()
def download_lead_summary_report(filters,user, col_list, download_report_id):
	"""
	this is the function for download lead summary reports
	"""	
	try:
		csv_file = []
		count=0
		...
		logged_in_user = User.objects.get(id=user)
		filter_by_camp = filters.get("filter_by_camp", [])
		start_date = filters.get("start_date", "")
		end_date = filters.get("end_date", "")
		download_type = filters.get('download_type',"")
		start_end_date_filter = Q(created_date__gte=start_date)&Q(created_date__lte=end_date)
		if filter_by_camp:
			campaign_list = Campaign.objects.filter(name__in=filter_by_camp)	
		else:
			if logged_in_user.is_superuser or str(logged_in_user.user_role) == 'admin':
				campaign_list = Campaign.objects.filter(status="Active")
			else:
				campaign_list = Campaign.objects.filter(status="Active").filter(created_by__id=logged_in_user.id)
		if "" in col_list:
			col_list.remove("")
		csv_file.append(col_list) 
		for campaign in campaign_list:
			...
			campaign_dict = Contact.objects.filter(campaign=campaign.name,phonebook_id__in=phonebook_ids).aggregate(Data=Count('id'),
				Dialed=Count('status',filter=Q(status__in=["Dialed", "NC", "Drop", "Invalid Number", "Queued-Callback", "Queued-Abandonedcall"])),
				Notdialed=Count('status',filter=Q(status="NotDialed")),Queuedcalls=Count('status',filter=Q(status="Queued"))
				)
			if start_end_date_filter:
				campaign_dict = Contact.objects.filter(campaign=campaign.name,phonebook_id__in=phonebook_ids).filter(start_end_date_filter).aggregate(Data=Count('id'),
				Dialed=Count('status',filter=Q(status__in=["Dialed", "NC", "Drop", "Invalid Number", "Queued-Callback", "Queued-Abandonedcall"])),
				Notdialed=Count('status',filter=Q(status="NotDialed")),Queuedcalls=Count('status',filter=Q(status="Queued"))
				)
			campaign_dict["Campaign"] = campaign.name
			campaign_dict['LoginAgents'] = 0
			if campaign.name in r_campaigns:
				campaign_dict['LoginAgents'] = len(r_campaigns[campaign.name])
			phonebook_data = phonebooks.aggregate(LeadList = Count('id'),
				ActiveList=Count('id',filter=Q(status='Active')))	
			campaign_dict.update(phonebook_data)
			campaign_dict['TotalConnectedcalls'] = CallDetail.objects.filter(campaign_name=campaign.name ,
				dialed_status='Connected').filter(Q(created__gte=start_date)&Q(created__lte=end_date)).count()
			row = []
			for field in col_list:
				row.append(campaign_dict.get(field,""))
			csv_file.append(row)
			count +=1
			percentage = ((count)/(campaign_list.count()))*100
			if download_report_id !=None: 
				set_download_progress_redis(download_report_id, round(percentage,2))
		
			####### To save file ###########		
			if download_type == 'xls':
				save_file(csv_file, download_report_id, 'lead_summary',logged_in_user, 'xls')
			else:
				save_file(csv_file, download_report_id, 'lead_summary',logged_in_user, 'csv')
	except Exception as e:
			if download_report_id != None:
				set_download_progress_redis(download_report_id, 100.0, is_refresh=True)
			exc_type, exc_obj, exc_tb = sys.exc_info()
...
def get_all_keys_data(team_extensions="",superadmin=False):
	keysdata = settings.R_SERVER.scan_iter("flexydial_*")
	agent_dict = {}
	if team_extensions:
		team_extensions =[bytes("flexydial_"+s, encoding='utf-8')  for s in team_extensions]
